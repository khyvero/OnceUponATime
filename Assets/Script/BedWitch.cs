...
public class BedWitch : MonoBehaviour, ICatalog, IPickable
{
    public const string ID = "bedWitch";

    IHolder holder;

    Vector3 pickPosition = new Vector3(0f, -1f, 0f);
    ...
    void ICatalog.ShowCatalog(Object caller)
    {
        if (holder == null)
        {
            CatalogUtil.ShowMessage("Bed - pick up : mouse left");
        }

        else if (holder != null)
        {
            CatalogUtil.ShowMessage("Bed - put down : mouse left");
        }
    }
