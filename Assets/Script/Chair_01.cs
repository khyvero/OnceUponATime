...
public class Chair_01 : MonoBehaviour, ICatalog, IPickable
{
    public const string ID = "chair";

    IHolder holder;

    Vector3 pickPosition = new Vector3(-8f, -65f, -5f);
    Vector3 pickRotation = new Vector3(0f, 130f, 0f);
    ...
    void ICatalog.ShowCatalog(Object caller)
    {
        if (holder == null)
        {
            CatalogUtil.ShowMessage("Chair  -  Pick Up : Mouse Left");
        }

        else if (holder != null)
        {
            CatalogUtil.ShowMessage("Chair  -  Put Down : Mouse Left");
        }
    }
